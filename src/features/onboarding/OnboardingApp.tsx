...
function PlanReady({
  ...
  return (
    <div className="mx-auto max-w-6xl rounded-[2rem] border border-ink/15 bg-white/80 p-6 shadow-soft sm:p-10">
      ...
      {plan && <div className="mt-8"><CalendarView events={combineCalendarEvents({ courses: data.courses, classSchedules: data.classSchedules, sessions: plan.sessions, academicEvents: data.academicEvents }, calendarRangeForPlan(plan.planningPeriod.start, plan.planningPeriod))} initialDate={plan.planningPeriod.start} range={calendarRangeForPlan(plan.planningPeriod.start, plan.planningPeriod)} initialMode="month" /></div>}
