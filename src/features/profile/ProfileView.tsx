...
const LOCAL_HISTORY_KEY = "planify:semester-history:v1";
...
export default function ProfileView({ data, onLogout }: { data: MainData; onLogout: () => void }) {
  ...
  const [history, setHistory] = useState<SemesterHistoryItem[]>(() => {
    ...
    try { return JSON.parse(window.localStorage.getItem(LOCAL_HISTORY_KEY) ?? "[]") as SemesterHistoryItem[]; } catch { return []; }
  });
  ...
  async function beginSemester() {
    ...
    if (!data.authenticated) {
      const name = `Semester Baru ${new Date().getFullYear()}/${String(new Date().getMonth() + 1).padStart(2, "0")}`;
      const next = createNewSemesterSetup(data.setup, reuse, name);
      const existing = { id: `local-${Date.now()}`, name: data.setup.semester, is_active: false, setup_payload: data.setup };
      const prior = (() => { try { return JSON.parse(window.localStorage.getItem(LOCAL_HISTORY_KEY) ?? "[]") as SemesterHistoryItem[]; } catch { return []; } })();
      window.localStorage.setItem(LOCAL_HISTORY_KEY, JSON.stringify([existing, ...prior]));
      saveLocalMainData(next);
