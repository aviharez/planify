...
import { groupMonthEvents, type MonthEventGroup } from "./transform";
...
const MONTH_CATEGORY_LABELS: Record<MonthEventGroup, string> = { class: "Kuliah", study: "Belajar", agenda: "Agenda", exam: "Ujian" };
const MONTH_CATEGORY_COLORS: Record<MonthEventGroup, string> = { class: "bg-moss", study: "bg-coral", agenda: "bg-amber-600", exam: "bg-rose-600" };
...
export default function CalendarView({ events, initialDate, range, initialMode = "week" }: { events: PlanifyCalendarEvent[]; initialDate: string; range: { start: string; end: string }; initialMode?: "week" | "month" }) {
  ...
  const monthGroups = useMemo(() => groupMonthEvents(events), [events]);
