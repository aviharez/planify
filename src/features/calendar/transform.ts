...
export function mapGoogleEventToCalendarEvent(event: GoogleCalendarEvent): PlanifyCalendarEvent | null {
  if (!event.id || event.extendedProperties?.private?.planifyManaged === "true") return null;
  const dateTime = event.start?.dateTime;
  const endDateTime = event.end?.dateTime;
  const date = (dateTime ?? event.start?.date ?? "").slice(0, 10);
  if (!date) return null;
  return { id: `google-${event.id}`, source: "google", category: "other", title: event.summary || "Kegiatan Google Calendar", date, startTime: dateTime?.slice(11, 16), endTime: endDateTime?.slice(11, 16), editable: false, details: event.description || undefined };
}
